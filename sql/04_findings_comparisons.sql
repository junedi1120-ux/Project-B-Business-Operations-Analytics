-- Finding comparison queries
-- Source: data/processed/project_b_simulated.sqlite
-- Snapshot date: 2026-08-31 (see docs/kpi_contract.md)
-- Read-only.

-- C1: Onboarding cycle time and completed-case SLA attainment by company size
...
-- C2: Open onboarding backlog and overdue count as of the snapshot
SELECT a.company_size,
       COUNT(*) AS open_cases,
...
-- C3: Snapshot Proposal-to-Won conversion by region and company size
...
-- C4: Closed win rate by logged-activity volume within deal-size band
-- Activities are aggregated to one row per opportunity first.
...
-- C5: Exploratory closed win rate by lead source
