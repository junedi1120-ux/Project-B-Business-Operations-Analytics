...
Build a decision-ready SQL and Power BI analysis for a simulated B2B sales and onboarding process. The project focuses on practical Business Operations questions: which metrics can be trusted, where management attention is required, and what actions can be tracked without overstating causality.
...
2. **Activity volume is not a reliable win-rate lever by itself.** For Large deals, closed win rate rises from 30.4% at ≤3 activities to 43.5% at four, then falls to 38.9% at 5+. Small deals show the opposite direction.
...
- Do not set an activity-count target. Add next-step owner, due-date, and blocker fields, then test a future Large-deal cohort.
- Review all 3 open APAC Enterprise proposals and the 25 losses. Treat 35% only as a sensitivity scenario, not a forecast or performance commitment.

### Validation and limitations

Dashboard KPIs were reconciled to SQL baselines. Activities were aggregated to one row per opportunity before outcome analysis. The data is entirely synthetic and deterministic; the three main findings confirm embedded mechanisms rather than independent real-world discoveries. Snapshot denominators and non-causal interpretations are disclosed. Estimated impacts are scenario calculations, not realised performance, revenue, or market benchmarks.
...
- KPI contract, findings, recommendations, and dashboard screenshots
